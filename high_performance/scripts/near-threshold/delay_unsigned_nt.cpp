...
int main()
{
	...
	// max supply 0.4
	string pwlh = " PWL (0n 0 2n 0 2.01n 0.4 4n 0.4)";
	string pwhl = " PWL(0n 0.4 2n 0.4 2.01n 0 4n 0)";
	...
	{ 
		...
		for (j = 0; j < 8; j++)
		{
			...
			// if j is the transition bit of an arch
			if (j == lista[i].difin)
			{
				// compose the supply with risein or riseout
				if (lista[i].risein == 1)
				{
			...
			// if not transition, use normal supply
			// supply 0V or 0.4V
			else
			{
		...
		// rise input, rise output
		if (lista[i].risein == 1 && lista[i].riseout == 1)
		{
			measure << "\tmeas tran TP_LH_" << lista[i].linha1 << "_" << lista[i].linha2 << " trig v(f" << lista[i].difin << ") val = 0.2 rise=1";
			measure << " targ v(p" << lista[i].numarc << ") val = 0.2 rise=1" << endl;
		...
		{
			measure << "\tmeas tran TP_HL_" << lista[i].linha1 << "_" << lista[i].linha2 << " trig v(f" << lista[i].difin << ") val = 0.2 fall=1";
			measure << " targ v(p" << lista[i].numarc << ") val = 0.2 fall=1" << endl;
			measure << "\techo TP_HL_" << lista[i].linha1 << "_" << lista[i].linha2 << " $&TP_HL_" << lista[i].linha1 << "_" << lista[i].linha2;
			measure << " >> Atrasos.txt" << endl
					<< endl;
		}
		// rise input, fall output
		else if (lista[i].risein == 1 && lista[i].riseout == 0)
		{
			measure << "\tmeas tran TP_HL_" << lista[i].linha1 << "_" << lista[i].linha2 << " trig v(f" << lista[i].difin << ") val = 0.2 rise=1";
			measure << " targ v(p" << lista[i].numarc << ") val = 0.2 fall=1" << endl;
			measure << "\techo TP_HL_" << lista[i].linha1 << "_" << lista[i].linha2 << " $&TP_HL_" << lista[i].linha1 << "_" << lista[i].linha2;
			measure << " >> Atrasos.txt" << endl
					<< endl;
		}
		// fall input, rise output
		else if (lista[i].risein == 0 && lista[i].riseout == 1)
		{
			measure << "\tmeas tran TP_LH_" << lista[i].linha1 << "_" << lista[i].linha2 << " trig v(f" << lista[i].difin << ") val = 0.2 fall=1";
			measure << " targ v(p" << lista[i].numarc << ") val = 0.2 rise=1" << endl;
			measure << "\techo TP_LH_" << lista[i].linha1 << "_" << lista[i].linha2 << " $&TP_LH_" << lista[i].linha1 << "_" << lista[i].linha2;
			measure << " >> Atrasos.txt" << endl
					<< endl;
		...
		// measure of energy all transition
		measure << endl
				<< "*------------------Energia----------------------------" << endl
				<< endl;

		measure << "\tmeas tran energia_" << lista[i].linha1 << "_" << lista[i].linha2 << " INTEG i(Vvdd) from=0n to=8n" << endl;
