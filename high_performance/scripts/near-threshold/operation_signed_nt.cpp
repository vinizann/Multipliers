...
int main()
{
	...
	// max supply 0.4
	string pwlh = " PWL (0n 0 2n 0 2.01n 0.4 8n 0.4)";
	string pwhl = " PWL(0n 0.4 2n 0.4 2.01n 0 8n 0)";
	...
	for (i = 0; i < 256; i++)
	{
		...
		measure << "*-------------------Operacoes--------------------------" << endl
				<< endl;
		measure << "\t"
				<< "meas tran s7 find v(p6) at=8n" << endl;
		measure << "\t"
				<< "meas tran s6 find v(p6) at=8n" << endl;
		measure << "\t"
				<< "meas tran s5 find v(p5) at=8n" << endl;
		measure << "\t"
				<< "meas tran s4 find v(p4) at=8n" << endl;
		measure << "\t"
				<< "meas tran s3 find v(p3) at=8n" << endl;
		measure << "\t"
				<< "meas tran s2 find v(p2) at=8n" << endl;
		measure << "\t"
				<< "meas tran s1 find v(p1) at=8n" << endl;
		measure << "\t"
				<< "meas tran s0 find v(p0) at=8n" << endl;
		measure << "\t"
		...
		measure << endl
				...
				<< endl;
		measure << "\t"
				<< "meas tran energia_" << i << " INTEG i(Vvdd) from=0n to=8n" << endl;
